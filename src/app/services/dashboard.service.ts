...
import { Dashboard_Aluno_Aula_Reposicao, Dashboard_Item_Status, Dashboard_Request, Dashboard_V2 } from '../models/dashboard-v2.model';

@Injectable({
    providedIn: 'root',
})
export class DashboardService extends Service {
    dashboard = new BehaviorSubject<Dashboard_V2>({ alunos: [], mesesRoteiro: [] });
    onReload = new EventEmitter<any>();

    getDashboard(request: Dashboard_Request) {
        return this.http.post<Dashboard_V2>(`${this.url}/eventos/dashboard-v2`, request)
            .pipe(map(res => {
