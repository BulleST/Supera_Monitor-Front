...
export class AlunoChecklistDialogComponent implements OnChanges {
  ...
  checkboxMark(alunoChecklistItem: Aluno_CheckList_Item, model: NgModel) {
    ...
    const onFinish = this.alunoChecklistOnConfirmDialog.onFinish.subscribe(async res => {
      ...
      // Instead of just updating the item, reload the full aluno object
      if (this.aluno?.id) {
        try {
          const updatedAluno = await this.alunoService.get(this.aluno.id).toPromise()
          if (updatedAluno) {
          ...
          console.error('Erro ao atualizar aluno após finalizar checklist item', e)
        }
      }
      onCancel.unsubscribe()
      onFinish.unsubscribe()
