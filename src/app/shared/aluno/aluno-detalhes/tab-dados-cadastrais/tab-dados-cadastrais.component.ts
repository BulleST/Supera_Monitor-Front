...
export class TabDadosCadastraisComponent implements OnChanges, OnDestroy {
	...
	jornadaAluno!: JornadaSupera_List_Aluno;
	...
	loadJornada() {
		...
		lastValueFrom(this.jornadaSuperaService.getJornadaAluno(this.aluno_Id))
			...
			.catch(res => {
				this.loadingJornada = false;
				this.toastr.error('Não foi possível carregar jornada do aluno');
			})
	}


	setJornadaAtual() {
		if (!this.object.id || !this.jornadaAluno) {
			return;
		}

		this.jornadaAtual = this.jornadaAluno.checklists.find(x => x.id == this.object.checklist_Id);

		const prazoFinal90Dias = moment(this.object.created).add(90, 'days');

		if (prazoFinal90Dias.isBefore(new Date, 'date')) {
			this.jornadaNome = '90 dias encerrados';

			if (this.jornadaAluno.checklists.find(x => x.status == JornadaSuperaStatus.Atrasado)) {
				this.jornadaNome = '90 dias encerrados com pendências';
				this.jornadaIcon = 'pi pi-times-circle text-red-500'
			}
			else if (this.jornadaAluno.checklists.find(x => x.status == JornadaSuperaStatus.FinalizadoComAtraso)) {
				this.jornadaNome = '90 dias encerrados com atraso';
				this.jornadaIcon = 'pi pi-times-circle text-red-500'
			}
		}
		else {
			if (this.jornadaAtual) {
				const finalizados = this.jornadaAtual.finalizados;
				const total = this.jornadaAtual.items.length;

				this.jornadaNome = this.jornadaAtual.nome;
				this.jornadaIcon = 'pi pi-exclamation-triangle text-orange-500'
				this.jornadaFinalizados = `${finalizados} / ${total}`
