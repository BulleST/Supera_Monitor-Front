...
import { OficinaInscricaoRequest, PseudoEvento } from '../../../../models/reposicao.model';
...
@Component({
	selector: 'app-agendar-oficina',
	standalone: false,
	templateUrl: './agendar-oficina.component.html',
	styleUrl: './agendar-oficina.component.css',
	providers: [ConfirmationService]

})
export class AgendarOficinaComponent implements OnInit, OnDestroy {
	visible: boolean = false
	subscription: Subscription[] = [];
	instance: DynamicDialogComponent | undefined;
	loading = false;
	maximized = false;
	view = new AgendarOficinaView;
	...
	sendConfirmation(e: any) {
		if (!this.aluno) {
			...
			this.toastrService.error('Selecione uma oficina')
		}
		else {
			let data = moment(this.evento.data).format('DD/MM/YY [às] HH[h]mm');
			this.confirmationService.confirm({
				target: e.target,
				message: `Tem certeza que deseja agendar oficina do aluno <b>${this.aluno.nome} </b> para o dia <b>${data}</b>?`,
	...
	async send(e: any, aluno: Aluno, evento: Evento) {
		...
		let request : OficinaInscricaoRequest = {
            aluno_Id: aluno.id,
            evento_Id: evento.id,
        }
		...
		if (request.evento_Id == PseudoEvento.EventoId) {
			response = await this.requestOficina(evento)
				.catch(res => {
					this.loading = false;
					return res;
				});
                ...
                if (!response.success) {
                    ...
                    return this.showError(
                        'Oficina não agendada',
                        `Ocorreu um erro ao agendar oficina. <br> ${response.message}`,
                        e,
                    )
                }
		...
		lastValueFrom(this.eventoService.inscrever(request.aluno_Id, request.evento_Id))
			.then(res => {
				if (res.success) {
					this.loading = false;
					...
					this.loading = false;
					this.showError('OPS', 'Não foi possível agendar a oficina.', e, res.message)
				}
	...
	requestOficina(evento: Evento) {
		return this.calendarioUtils.requestOficina(evento);
	}
	...
	sendMensagemAluno(e: any, evento: Evento) {
		...
		this.confirmationService.confirm({
			target: e.target,
			message: `Oficina agendada com sucesso. <br> Clique para enviar mensagem de confirmação.`,
...
export class AgendarOficinaView {
	aluno?: Aluno;
	evento?: Evento;
}