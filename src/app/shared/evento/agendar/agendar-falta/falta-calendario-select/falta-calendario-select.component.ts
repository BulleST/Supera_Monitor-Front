import { Input, signal, Component, OnChanges, OnDestroy, ViewChild, SimpleChanges, ChangeDetectorRef, Output, EventEmitter, AfterViewInit } from '@angular/core'
...
@Component({
	...
	standalone: false,
	templateUrl: './falta-calendario-select.component.html',
	styleUrl: './../agendar-falta.component.css',
	providers: [ConfirmationService],

})
export class FaltaCalendarioSelectComponent implements OnDestroy, AfterViewInit {
	...
	initCalendar() {
		...
		if (this.aluno) {
			this.calendarioRequest.aluno_Id = this.aluno.id;
			this.update()
		}
	}
	...
	update() {
		this.loading = true;
		var api = this.fullCalendar.getApi()
		var view = api.view;
		this.calendarioRequest.intervaloDe = view.currentStart;
		this.calendarioRequest.intervaloAte = view.currentEnd;
		this.getCalendario()
			.then(res => {
				this.setCalendario()
				this.loading = false;
	...
	prev() {
		var api = this.fullCalendar.getApi()
		api.prev()
		this.data = api.view.currentStart;
		this.update();
	}
		...
		return lastValueFrom(this.service.getList(this.calendarioRequest))
			.then(res => {
				...
				this.eventos = res.eventos.filter(evento => {
					...
					const eventoNaoFinalizado = evento.finalizado == false;
					// const eventoEhAula = [EventoTipo.Aula, EventoTipo.TurmaExtra].includes(evento.evento_Tipo_Id);
					const alunoEstaNaAula = evento.alunos.find(x => x.aluno_Id == aluno.id);
					const dataDesativado = !aluno.deactivated || moment(aluno.deactivated).isSameOrBefore(evento.data);
