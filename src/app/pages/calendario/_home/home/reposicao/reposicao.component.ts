...
import { PseudoAula, ReposicaoAluno, ReposicaoAlunoRequest } from '../../../../models/reposicao.model';
...
export class ReposicaoComponent implements OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        this.activatedRoute.queryParams.subscribe(async res => {
            this.object = {
                ...
                source_Sala_Id: res['source_Sala_Id'],
            } as ReposicaoAluno
    ...
    confirmaReposicao(e: any) {
        ...
        var target = this.selectedAula!.extendedProps as CalendarioAula;
    ...
    async send(target: CalendarioAula, e: any) {
        ...
        if (this.object.source_Aula_Id == PseudoAula.AulaId) {
            var aulaRequest: AulaCreateRequest = {
                ...
                observacao: '',
                descricao: this.object.source_Turma ?? '',
                perfilCognitivo: this.turma.perfilCognitivo, 
            }
            console.log('aula source', aulaRequest)
            await lastValueFrom(this.service.create(aulaRequest))
                .then(res => {
                    reposicaoRequest.source_Aula_Id = res.object.aula_Id;
                    this.object.source_Aula_Id = res.object.aula_Id;
                })
                .catch(res => this.showError('Ocorreu um erro', `Não foi possível agendar reposição. \n (Aula source não foi inserida). \n ${getError(res)}`, e));
        } else {
            reposicaoRequest.source_Aula_Id = this.object.source_Aula_Id;
        }
