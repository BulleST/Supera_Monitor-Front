  ...
  alunoChanged(e: MultiSelectChangeEvent, model: NgModel) {
    ...
    if (selected == false) {
      ...
      if (aluno && aluno.disponivel == false && aluno.disponivelEvent) {
        ...
        this.showError(
          'Aluno Indisponível',
          `${nome} tem ${this.getTipo(aluno.disponivelEvent)} no mesmo dia às <b>${moment(
            aluno.disponivelEvent.data,
          ).format('HH[h]mm')}</b>.`,
          e.originalEvent,
        )
        return
        ...
        this.showError(
          'Restrição de Mobilidade',
          `O ${nome} tem restrição de mobilidade e não pode participar da aula zero na sala ${salaAula.numeroSala} - ${salaAula.andar}º andar.`,
          e.originalEvent,
        )
        return
