...
export class EditarAulaComponent implements OnChanges, OnDestroy {
  ...
  async setApostilasAlunos() {
    ...
    this.evento.alunos.forEach((aluno) => {
      if (aluno.apostila_Abaco_Id) {
        aluno.apostilaAbacoObject = this.apostilas.find(
          (x) => x.id == aluno.apostila_Abaco_Id,
        ) as Apostila
        aluno.apostilasAbacoList = this.apostilas.filter(
          (x) =>
            x.apostila_Kit_Id == aluno.apostila_Kit_Id &&
            x.apostila_Tipo_Id == ApostilaTipo.Abaco,
        )
        aluno.numeroPaginaAbaco = aluno.numeroPaginaAbaco ?? 0
      }

      if (aluno.apostila_AH_Id) {
        aluno.apostilaAHObject = this.apostilas.find(
          (x) => x.id == aluno.apostila_AH_Id,
        ) as Apostila
        aluno.apostilasAHList = this.apostilas.filter(
          (x) =>
            x.apostila_Kit_Id == aluno.apostila_Kit_Id &&
            x.apostila_Tipo_Id == ApostilaTipo.AH,
        )
  ...
  apostilaAbacoChange(item: Evento_Participacao_Aluno, e: SelectChangeEvent) {
    ...
    let oldApostila = this.clonedRow[item.aluno_Id]
      .apostilaAbacoObject as Apostila
  ...
  numeroPaginaAHChange(item: Evento_Participacao_Aluno, e: any) {
    ...
    if (
      current.numeroPaginaAH <= prev.numeroPaginaAH &&
      prev.apostila_AH_Id == current.apostila_AH_Id
    ) {
      this.confirmationService.confirm({
        target: e.target,
  ...
  goToReposicao() {
    if (this.evento) {
      ...
      this.router.navigate(
        ['aula', 'reposicao', this.crypto.encrypt(this.evento.id)],
