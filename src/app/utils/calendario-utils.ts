...
export class CalendarioUtils {
  ...
  getEventStyles(item: Evento): {
    ...
    switch (item.evento_Tipo_Id) {
      ...
      default:
        backgroundColor =
          item.corLegenda ?? item?.professores[0]?.corLegenda ?? DEFAULT_COLOR
  ...
  async cancelarEventoAutomaticamente(evento: Evento, feriado: Feriado) {
    ...
    if (evento.id == PseudoEvento.EventoId) {
      await this.request(evento).then((res) => {
    ...
    if (response.success) {
      ...
      response = await lastValueFrom(this.service.cancelar(request)).then(
        (res) => {
          evento.active = false
