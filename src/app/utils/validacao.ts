...
export function validaSalaAulas(data: Date, duracaoMinutos: number, salaAulas: SalaAula[], eventos: Evento[], turma_Id?: number, evento_Id?: number) {
    ...
    console.log('intervaloDe', intervaloDe.format('DD/MM HH:mm'))
    console.log('intervaloAte', intervaloAte.format('DD/MM HH:mm'))
    console.log('duracaoMinutos', duracaoMinutos)
    console.log('evento_Id', evento_Id)
    console.log('turma_Id', turma_Id)
    console.log('salaAulas', salaAulas)
    ...
    return salaAulas.map(item => {
        ...
        console.group(item.id, 'Sala:' + item.numeroSala, item.andar + ' º andar')
        let evento = eventos.find(e => {
            let eventoIntervalo = [moment(e.data), moment(e.data).add(e.duracaoMinutos, 'minutes')]

            console.group('evento', e.descricao, moment(e.data).format('DD/MM HH:mm'))

            let c1 = turmaIntervalo[0].isAfter(eventoIntervalo[0], 'minutes')
            let c2 = turmaIntervalo[0].isBefore(eventoIntervalo[1], 'minutes')
            let c3 = turmaIntervalo[1].isAfter(eventoIntervalo[0], 'minutes')
            let c4 = turmaIntervalo[1].isBefore(eventoIntervalo[1], 'minutes')
            let c5 = e.sala_Id == item.id;
            let c6 = turma_Id ? e.turma_Id != turma_Id : true;
            let c7 = evento_Id ? e.id != evento_Id : true;
            let c8 = e.active;
            let c9 = ((c1 && c2) || (c3 && c4)) && c5 && c6 && c7 && c8;
